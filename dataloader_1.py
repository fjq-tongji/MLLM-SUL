    ...
    def __init__(self, db_path, ext, feature_type):
        ...
        if self.ext == '.npy':
            ...
            if 'cocotest' in self.db_path:
                ...
                elif self.feature_type == 'grid_feats':
                    self.loader = lambda x: np.load(x)['features']   ###['g_feature'] 是global特征
                elif self.feature_type == 'global_feats':
                    self.loader = lambda x: np.load(x)['g_feature']   ###['g_feature'] 是global特征
...
class DataLoader(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        if self.opt.input_label_h5 != 'none':
            ...
            seq_size = self.h5_label_file['labels'].shape    #### 16
            self.tokens = self.h5_label_file['labels'][:]
        ...
        self.num_images = len(self.info['images']) # self.tokens_start_ix.shape[0]
    ...
    def get_batch(self, split, batch_size=None):
        ...
        for i in range(batch_size):
            # fetch image
            tmp_fc, tmp_att, tmp_grid_feats, tmp_global_feats, tmp_seq, ix, tmp_wrapped = self._prefetch_process[split].get()                 ##************************************
            if tmp_wrapped:
                wrapped = True
        ...
        if self.use_box:
            fc_batch, att_batch, grid_feats_batch, global_feats_batch, tokens_batch, gts, infos = \
                zip(*sorted(zip(fc_batch, att_batch, grid_feats_batch, global_feats_batch, tokens_batch, gts, infos), key=lambda x: 0, reverse=True))
        else:
            fc_batch, att_batch, grid_feats_batch, global_feats_batch, tokens_batch, gts, infos = \
                zip(*sorted(zip(fc_batch, att_batch, grid_feats_batch, global_feats_batch, tokens_batch, gts, infos), key=lambda x: 0, reverse=True))
    ...
    def __getitem__(self, index):   ###迭代器
        ...
        if hasattr(self, 'h5_label_file'):
            tokens = self.get_captions(ix, self.seq_per_img)      ####(5,16) int
        else:
            tokens = None

        if self.use_box:
            return (fc_feat,
                    att_feat, grid_feats, global_feats, boxes_feat, tokens,           ##************************************
                    ix)
        else:
            return (fc_feat,
                    att_feat, grid_feats, global_feats, tokens,  ##************************************
                    ix)
