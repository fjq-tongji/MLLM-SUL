id="transformer_1218_llama3.2_3B"
if [ ! -f log/log_$id/infos_$id.pkl ]; then
start_from=""
else
start_from="--start_from log/log_$id"
fi
python train_drama_llama3.2_3B.py --id transformer_llama3  \
    --caption_model transformer_llama3  \
    --refine 1 \
    --refine_aoa 1 \
    --use_ff 0 \
    --decoder_type AoA \
    ...
    --dropout_aoa 0.3 \
    --label_smoothing 0.2 \
    --pretrained_llama /data/fjq/LLAMA/llama-3.2-3b/consolidated.00_1.pth  \
