id="transformer_drama_cap_blip2_27b"
if [ ! -f log/log_$id/infos_$id.pkl ]; then
start_from=""
else
start_from="--start_from log/log_$id"
fi
python train_drama_1-cap-blip2.py --id transformer_blip2  \
    --caption_model transformer_blip2 \
    --refine 1 \
    --refine_aoa 1 \
    --use_ff 0 \
    --decoder_type AoA \
    ...
    --input_json data/drama/blip2/drama_dataset/drama_llama_adapter_DRAMA.json \
    --input_label_h5 data/drama/blip2/drama_dataset/drama_llama_adapter_DRAMA.h5 \
    --h5_reg  True   \
