...
class Decoder_llama(nn.Module):
    def __init__(self, layer):
        ...
        #self.opt_model = '/data/fjq/blip-2/facebook/opt-2.7b'
    ...
    def forward(self, memory, global_feats_, src_mask, tgt, tgt_mask):
        ...
        inputs_embeds_ = inputs_embeds         ##(b,100,768)

        #------------------------------------------------------------------------------------------------#
        query_tokens = self.query_tokens.expand(memory.shape[0], -1, -1)
        memory_ = torch.cat([memory, inputs_embeds_], dim=1)              ###(b,49+100,768)

        image_atts = torch.ones(memory_.size()[:-1], dtype=torch.long).to(memory_.device)

        query_output = self.Qformer.bert(
            query_embeds=query_tokens,
            encoder_hidden_states=memory_,
            encoder_attention_mask=image_atts,
            use_cache=True,
            return_dict=True,
        )

        inputs_opt = query_output.last_hidden_state           ###(b,32,768)
        #print(inputs_opt.shape)
        # ---------------------------------------------------------------------------------------------#
        memory_2 = inputs_opt
        inputs_embeds_2 = torch.cat([memory_2, inputs_embeds], dim=1)   ###(b,32+max_len,768)

        # atts_opt = torch.ones(memory_2.size()[:-1], dtype=torch.long).to(memory.device)           ###(b,32)
        # opt_attention_mask = torch.ones(inputs_embeds_.size()[:-1], dtype=torch.long).to(inputs_embeds_.device)  ###(b,max_len)
        attention_mask = torch.ones(inputs_embeds_2.size()[:-1], dtype=torch.long).to(inputs_embeds_2.device)  ###(b,32+max_len)

        outputs = self.opt_model.model.decoder(                         ###outputs[0]: (b,32+max_len,768)
            inputs_embeds=inputs_embeds_2, attention_mask=attention_mask
        )
        outputs_ = outputs
    ...
    def forward_inference(self, memory, tokens):
        ...
        inputs_embeds = self.opt_model.model.decoder.embed_tokens(tokens[:, 0:50])     ###tokens=(b,问题) -> (b,问题,2560)
        ...
        inputs_embeds = torch.cat([memory_2, inputs_embeds], dim=1)  ###(b,32+问题,2560)
        ...
        outputs = self.opt_model.generate(               ### (b,50)
            inputs_embeds=inputs_embeds, max_length=35, do_sample=True, top_p=0.75, temperature=0.5, repetition_penalty=1.0,
...
class TransformerModel_instructblip2(AttModel):
    ...
    def __init__(self, opt):
        super(TransformerModel_instructblip2, self).__init__(opt)
        self.opt = opt
    ...
    def get_trainable_params(self):
        ...
        #train_param_name = ['decoder_llama.reg_token_2', 'decoder_llama.vl_pos_embed',
         #                   'decoder_llama.reg_layer', 'decoder_llama.bbox_embed_2', 'decoder_llama.ln',
          #                  'decoder_llama.fc'
           #                 ]
