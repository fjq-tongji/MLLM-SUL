...
class Encoder(nn.Module):
    ...
    def __init__(self, layer, N):
        ...
        self.opt_proj = nn.Linear(self.size, 4096)

    def forward(self, att, grid_feats, global_feats, semantic_feats, mask):
        grid_feats = self.opt_proj(grid_feats)
...
class Decoder_llama(nn.Module):
    def __init__(self, layer):
        ...
        self.llava_model_path = '/data/fjq/blip-2/llava-v1.5-7b'
        self.llava_model = LlamaForCausalLM.from_pretrained(
            self.llava_model_path #, torch_dtype=torch.float16
        )
    ...
    def forward(self, memory, global_feats_, src_mask, tgt, tgt_mask):   ##memory=global_feats_=grid_feats=(b,49,4096)
        _bsz, seqlen = tgt.shape               ###########(b,100)

        inputs_embeds = self.llava_model.model.embed_tokens(tgt)       ###tokens=(b,问题+答案=100) -> (b,问题+答案,4096)
        inputs_embeds_ = inputs_embeds
        inputs_embeds = torch.cat([memory, inputs_embeds], dim=1)              ###tokens=(b,49+100) -> (b,49+100,4096)

        # atts_opt = torch.ones(memory.size()[:-1], dtype=torch.long).to(memory.device)  ###(b,32)
        # opt_attention_mask = torch.ones(inputs_embeds_.size()[:-1], dtype=torch.long).to(
        #     inputs_embeds_.device)  ###(b,max_len)
        # attention_mask = torch.cat([atts_opt, opt_attention_mask], dim=1)

        attention_mask = torch.full((_bsz, 1, 149, 149), fill_value=1)
        attention_mask = torch.triu(attention_mask, diagonal=0 + 1).type_as(inputs_embeds)

        layers = self.llava_model.model.layers
        x = layers[0](hidden_states=inputs_embeds, attention_mask=attention_mask)[0]
        for layer in layers[1:]:
            x = layer(hidden_states=x, attention_mask=attention_mask)[0]

        outputs = x
        outputs = self.llava_model.lm_head(outputs)
    ...
    def forward_inference(self, memory, tokens):
        ...
        inputs_embeds = self.llava_model.model.embed_tokens(tokens[:, 0:64])  ###tokens=(b,问题+答案=100) -> (b,问题+答案,4096)
        inputs_embeds_ = inputs_embeds
        inputs_embeds = torch.cat([memory, inputs_embeds], dim=1)  ###tokens=(b,49+100) -> (b,49+100,4096)
        ...
        outputs = self.llava_model.generate(               ### (b,50)
...
class TransformerModel_llava(AttModel):
    ...
    def __init__(self, opt):
        super(TransformerModel_llava, self).__init__(opt)
        self.opt = opt
    ...
    def get_trainable_params(self):
        ...
        train_param_name = ['decoder_llama.llava_model'
                            ]
        for name, para in self.named_parameters():
            #print(name)
