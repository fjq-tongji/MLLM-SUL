...
class Decoder_llama(nn.Module):
    ...
    def forward(self, memory, global_feats_, src_mask, tgt, tgt_mask):
        ...
        h = torch.cat([memory, h], dim=1)  ###(b,L+49,4096)
        freqs_cis = self.llama.freqs_cis.to(h.device)
        freqs_cis = freqs_cis[:49+seqlen]

        mask = torch.full((1, 1, 49+seqlen, 49+seqlen), float("-inf"), device=h.device)
        mask = torch.triu(mask, diagonal=0 + 1).type_as(h)

        for layer in self.llama.layers:      ### 1层
            h = layer(h, 0, freqs_cis, mask)

        h = self.llama.norm(h)                             ####(b,49+L,4096)
        h_ = h
        ...
        #    h_ = layer(h_, mask=None)                  ###(b,1+144+36,512)
    ...
    def forward_inference(self, memory, tokens, start_pos: int):
        ...
        for layer in self.llama.layers:                        ### 32层
            h = layer(h, start_pos, freqs_cis, mask=mask)
...
class TransformerModel_llava(AttModel):
    ...
    def get_trainable_params(self):
        ...
        train_param_name = ['decoder_llama.llama'
                            ]
        for name, para in self.named_parameters():
            #print(name)
